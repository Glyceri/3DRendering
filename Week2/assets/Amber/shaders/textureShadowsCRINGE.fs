...
vec3 HandleAsDirectional(vec3 lightDir, float lampStrength, vec3 baseColour){
	float lightStrength = 1-max(dot(normalize(fNormal), normalize(lightDir)), 0.0f);
...
void main (void) {
	...
	for(int i = 0; i < LIGHTCOUNT; i++){
		...
		//float shadow = 0;
		//if(el.lightType == 0 && counter == 0) {shadow = ShadowCalculation(FragPosLightSpace, el.lightDirection); counter++; }
