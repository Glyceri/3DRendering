...
std::map<std::string, std::string> Mesh::ReadMTL(std::string fileName)
{
	std::cout << "READING MTL: " << fileName << std::endl;
	std::map<std::string, std::string> _mapToFile;
	...
	if (file.is_open()) {
		...
		while (getline(file, line)) {
			...
			if (strcmp(cmd, "newmtl") == 0) {
				char s[100];
				sscanf(line.c_str(), "%10s %s\n", cmd, &s);
				lastNewMTL = s;
			}
			else if (strcmp(cmd, "map_Kd") == 0) {
				char s[300];
				sscanf(line.c_str(), "%10s %s\n", cmd, &s);
				_mapToFile.emplace(lastNewMTL, s);
			}
		}
	}
	file.close();
...
std::vector<Mesh*> Mesh::loadMulti(std::string pFileName)
{
	std::cout << "Loading " << pFileName << "..." << std::endl;
	...
	if (file.is_open()) {
		...
		std::map<std::string, std::string> findPath;
		...
		while (getline(file, line)) {
			...
			if (strcmp(cmd, "mtllib") == 0) {
				...
				std::vector<std::string> filePaths = split(pFileName, '/');
				int size = filePaths.size() - 2;
				int counter = 0;
				std::string mtlFile = "";
				for (auto it = filePaths.begin(); it != filePaths.end(); ++it) {
					if (counter <= size) mtlFile += *it + "/";
					else mtlFile += s;
					counter++;
				}
				findPath = ReadMTL(mtlFile);
				...
				if (!createdBefore) {
					createdBefore = true;
					currentMesh->preferredFile = findPath[s];
					continue;
				}
				...
				if (count == 10) {
					...
					for (int i = 0; i < 3; ++i) {
						...
						if (found == mappedTriplets.end())
						{
							...
							currentMesh->_indices.push_back(index);
							...
							currentMesh->_vertices.push_back(vertices[vertexIndex[i] - 1]);
							currentMesh->_normals.push_back(normals[normalIndex[i] - 1]);
							currentMesh->_uvs.push_back(uvs[uvIndex[i] - 1]);
						}
						else
						{
