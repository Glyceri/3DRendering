...
private:
	...
	public:
		...
		static std::vector<Mesh*> loadMulti(std::string pFileName);
		static Mesh* load(std::string pFilename);

		std::string preferredFile = "error.png";

		static std::map<std::string, std::string> ReadMTL(std::string fileName);
        /**
	...
	protected:
		...
		//std::vector<std::string> _matNames;		//materials
